...
Day 11 - Optimizing a Neural Network
- Part 2
...
You might’ve heard ‘repeat the process
of Gradient Descent (GD) until convergence’.
But what is exactly convergence?
...
The real challenge lies in skipping the local minima to achieve global minima and
that is when you hit gold.
You can go through the below article to understand local minimas in neural network training:
https://www.allaboutcircuits.com/technical-articles/understanding-local-minima-
...
slope. Let’s look into them one by one.
Batch Gradient Descent
and only understand the concept.
You can only watch till 7:48 of the below video if you don’t wish to delve into the implementation
Stochastic and Batch Gradient Descent in practice
Link to the notebook used by the instructor in the previous video: https://github.com/codebasics/py/blob/master/DeepLearningML/8_
sgd_vs_gd/gd_and_sgd.ipynb
Batch Gradient Descent
...
Look at how smooth the graph is. It is because we are taking an average of gradients before taking a step.
...
As it uses one training example in every iteration this algo is faster for larger data set. Still confused why?
making a weight update. It can quickly process 1 example, make an update
needs to process a number of examples(a batch) together at a time before
It is because the network no longer
and then move to the next.
results are faster.
In SGD, one might not achieve the best accuracy, but the computation of
...
processed by the network at a time.
...
used. It is known as Mini-batch Gradient Descent.
To tackle this problem, a mixture of Batch Gradient Descent and SGD is
...
Remember how we didn’t use Sigmoid or Softmax in the hidden layers
and used ReLU instead? Now you’ll understand why.
...
(especially for networks that are not too deep).
it is relatively robust to the vanishing/exploding gradient issue
Using RELU/ leaky RELU as the activation function is a better choice, as
...
https://medium.com/fintechexplained/neural-networks-bias-
...
The weight initialization technique you choose for your neural network can determine how quickly the network converges or whether it converges at all. Although the initial values of these weights are just one parameter among many to tune, they are incredibly important.
Their distribution affects the gradients and, therefore, the effectiveness of training.
...
Zero initialization
Zero initialization serves no purpose. The neural net does not perform
symmetry-breaking.
...
perform well.
initializing them to 0 but is still not able to break symmetry and
Weights can be initialised to 1 too. That does perform better than
Random initialization
...
1. Weights should have good variance
...
Slide Download Link
You can download the slides here: https://docs.google.com/presentation/d/1XrIo4jdvqQZYh7zOaOAM7 pz1r54KLDGTulZkcpkJN0U/edit?usp=sharing
References
https://towardsdatascience.com/batch-mini-batch-stochastic-
gradient-descent-7a62ecba642a
https://www.wandb.com/articles/the-effects-of-weight-initialization- on-neural-nets
That’s it for the day. Thank you!
Feel free to post any queries on the Discuss forum or #help channel on Slack